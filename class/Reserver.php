...
class Reserver extends DbConnection{
    ...
    public function setReservation($idUser, $idVoiture, $dateDebut, $dateFin){
        ...
        if ($this->checkAvailability($idVoiture, $dateDebut, $dateFin)) {
    ...
    public function getReservationHistorique($idUser){
        $historique = [];
        $getReservations = $this->getBdd()->prepare("SELECT * FROM Reservations WHERE id_user = :id_user ORDER BY DateReservationDebut DESC");
    ...
    private function checkAvailability($idVoiture, $dateDebut, $dateFin){
        ...
        return !$result; // Retourne vrai si aucune réservation n'existe pour cette période
    }
}
